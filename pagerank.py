...
class WebGraph():
    ...
    def __init__(self, filename, max_nnz=None, filter_ratio=None):
        ...
        #print(self.url_dict)
        #print(self.url_dict.items())
        #print(self.url_dict.items()[0][1])
        #print(self.url_dict.values())
        #print(self.url_dict.keys())
        #ight = self.url_dict
        #print(ight)
        #print(ight.type())
        #print(_index_to_url(1))
    ...
    def search(self, pi, query='', max_results=10, sumts_weight=.03, power=30):
        n = self.P.shape[0]
        k = min(max_results,n)
        ...
        for i in range(n): #lets increase each url's pagerank proportionally if qearies, or words similar to queries, appear in the url
            my_n = 0 #the number of hits a url gets for given query 
            score = 0  
            sumts = 0 #the cumlative weights for how important a given word is 
            #print("now here")
            urll = self._index_to_url(i)

            if url_satisfies_query_notsim(urll,query): #when the query appears in the url, lets boost our parameters
                my_n += 1
                sumts += sumts_weight #a weight for how much we care about the appearce of the query in the URL
                                # this can help weigh terms mentioning the query higher since url_satisfies_query returns true equally for queries and synomyns 
            
            for word in range(10):
                #print("fin")
                w = S[word][0] 
                
                if url_satisfies_query_notsim(urll, w): #when a word similar to query appears in the url, lets boost its score! 
                    my_n += 1
                    sumts += S[word][1]**power
            
            
            score += my_n * sumts #for all the 'hits' my_n, lets increase score propotional to sumts
            
            #print(score)
            #print(pi[i])
            
            pi[i] = pi[i] + score #increase pagerank according to score 
            #print(pi[i])
            #print(i)
            #print(urll)
...
def url_satisfies_query_notsim(url, query):
    satisfies = False
    terms = query.split()
    '''
    syns = vectors.most_similar(args.search_query)


    for i in range(5):
        terms.append(syns[i][0])
    '''
...
if __name__=='__main__':
    ...
    parser.add_argument('--sumts_weight', type=float, default=.03)
    ...
    g.search(pi, query=args.search_query, max_results=args.max_results, sumts_weight=args.sumts_weight, power=args.power)
