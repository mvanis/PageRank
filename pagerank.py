    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            for k in range(max_iterations):
                ...
                #print("alpha_x0.shape=", alpha_x0.shape)
                
                piece_1 = ((torch.sparse.mm(self.P.t(), alpha_x0)).t()).t()
                
                #print("piece_1.shape = ", piece_1.shape)
                
                v = torch.unsqueeze(v, 1)
                
                a = torch.unsqueeze(a,1)
                
                piece2 = ((alpha_x0.t() @ a) + (1-alpha) )@ v.t()
                ...
                #print("piece2 = ",piece2 )
                #print("piece2 shape= ",piece2.shape )
                ...
                #print("x0 finally is", x0 )
                
                if torch.norm(x0 - x_minus1) < epsilon:
                    x0 = torch.squeeze(x0)
                    print("found epsilon")
                    return x0  
                k += 1
    ...
    def search(self, pi, query='', max_results=10):
        ...
        for i in range(n):
            ...
            if url_satisfies_query(url,query):
                print(f'rank={matches} pagerank={pagerank:0.4e} url={url}')
                matches += 1
